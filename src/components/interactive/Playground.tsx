...
export default function Playground() {
  ...
  const endpointsMap: {
    [key: string]: { description: string; mockResponse: any };
  } = {
  ...
  return (
    <div
      ...
      <div
        className={[
          // SURFACE
          "border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-[#0f111a] shadow-sm",
          // SPACING
          "p-5 space-y-5",
          // TRANSITION
        ...
        <div className="flex items-center justify-between border-b border-slate-200/50 dark:border-slate-800/50 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-800 dark:text-slate-500">
              PLAY YOUR API
            </span>
          </div>
        ...
        {/* 1. ENDPOINT SELECTOR */}
        <div>
          <label className="block text-slate-600 dark:text-slate-400 font-medium mb-1.5 uppercase">
            Pilih endpoint API:
          </label>
          <select
            value={endpoint}
            ...
            className={[
              ...
              "px-3 py-2",
              // SURFACE
              "border border-slate-200 dark:border-slate-800 rounded-md bg-slate-50 dark:bg-slate-900",
              // TYPOGRAPHY
              "text-slate-800 dark:text-slate-200 font-sans",
              // INTERACTION
              "cursor-pointer focus:outline-none focus:ring-1 focus:ring-slate-400 dark:focus:ring-slate-600",
            ].join(" ")}>
            <option value="GET /api/v1/scaffold-data">
              GET /api/v1/scaffold-data
            </option>
        ...
        <div>
          ...
          <textarea
            ...
            className={[
              ...
              "border border-slate-200 dark:border-slate-800 rounded-md bg-slate-50 dark:bg-slate-900",
              // TYPOGRAPHY
              "text-slate-800 dark:text-slate-200 font-sans text-sm",
              // INTERACTION
              "focus:outline-none focus:ring-1 focus:ring-slate-400 dark:focus:ring-slate-600 resize-none",
            ].join(" ")}
        ...
        <div className="border border-slate-100 dark:border-slate-900 rounded-lg overflow-hidden">
          <button
            ...
            className={[
              ...
              "px-4 py-3",
              // SURFACE
              "bg-slate-50 dark:bg-slate-900/60 border-b border-slate-100 dark:border-slate-900",
              // TYPOGRAPHY
          ...
          {isAdvancedOpen && (
            <div
              className={[
                ...
                "p-4 space-y-3",
                // SURFACE
                "bg-slate-50/20 dark:bg-slate-950/20 border-b border-slate-100 dark:border-slate-900",
              ].join(" ")}>
              <div className="flex items-center justify-between">
                <span className="text-slate-600 dark:text-slate-400">
                  [strict_diagnostics_flag]:
                </span>
                <input
                  type="checkbox"
                  defaultChecked
                  className="rounded border-slate-300 dark:border-slate-700 text-slate-500 focus:ring-slate-500 h-3.5 w-3.5"
                />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-600 dark:text-slate-400">
          ...
          {isJsonOpen && (
            <div
              className={[
                ...
                "p-0",
                // SURFACE
                "bg-slate-950 text-slate-300 border-b border-slate-100 dark:border-slate-900",
                // INTERACTION
                "select-all",
              ].join(" ")}>
        ...
        <button
          ...
          className={[
            ...
            "px-4 py-3",
            // SURFACE
            "rounded-lg bg-slate-950 dark:bg-slate-50",
            // TYPOGRAPHY
            "text-slate-50 dark:text-slate-950 font-medium tracking-wide text-center",
            // INTERACTION
            "cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-100 disabled:bg-slate-300 dark:disabled:bg-slate-800",
            // TRANSITION
            "transition-colors",
          ].join(" ")}>
          <Play
      ...
      <div
        className={[
          ...
          "p-5",
          // SURFACE
          "border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-900 dark:bg-[#090b11] shadow-inner",
          // TYPOGRAPHY
          "text-slate-300",
          // TRANSITION
          "transition-colors duration-200",
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4 text-[10px] text-slate-500 font-mono">
            ...
            {hasExecuted && (
              <span className="text-slate-400">
                Ping: ~{responseTime}ms STATUS: 200_OK
              </span>
            )}
          </div>
          ...
          <div className="overflow-x-auto max-h-[300px]">
            {isExecuting ? (
              ...
              <div className="py-20 text-center text-slate-600 flex flex-col items-center justify-center gap-2">
                ...
                <span>
                  Disini akan ditampilkan bentuk respon sesuai request POST/ GET
                </span>
              </div>
            )}
          </div>
        ...
        {hasExecuted && !isExecuting && (
          <div className="border-t border-slate-800 pt-3 mt-4 flex justify-between items-center">
            <span className="text-[10px] text-slate-500">
              SIZE: ~{new Blob([responseJson]).size} Bytes
            </span>
            <button
              onClick={handleCopy}
              className={[
                ...
                // INTERACTION
                "cursor-pointer hover:bg-slate-900 hover:text-slate-105",
                // TRANSITION
                "transition-colors",
              ].join(" ")}>
              {copied ? (
